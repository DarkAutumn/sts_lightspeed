...
void BattleContext::useAttackCard() {
    ...
    switch (c.getId()) {
        ...
        case CardId::CHOKE: {
            const int dmg = calculateCardDamage(c, t, 12);
            addToBot( Actions::AttackEnemy(t, dmg) );
        ...
        case CardId::BRILLIANCE: {
            // Java: deal (12 + mantraGained) damage, +4 on upgrade
            // (= 16 + mantraGained). mantraGained tracks total MANTRA
            // buffed in this combat.
            const int base = up ? 16 : 12;
            const int dmg = calculateCardDamage(c, t, base + mantraGained);
            addToBot( Actions::AttackEnemy(t, dmg) );
            break;
        }
        ...
        case CardId::ERUPTION: {
            const int dmg = calculateCardDamage(c, t, 9);
            addToBot( Actions::AttackEnemy(t, dmg) );
        ...
        case CardId::FLURRY_OF_BLOWS: {
            const int dmg = calculateCardDamage(c, t, up ? 6 : 4);
            addToBot( Actions::AttackEnemy(t, dmg) );
            // Note: on-stance-change return-to-hand is implemented in the
            // stance-change trigger hook (CardManager::onStanceChange or
            // equivalent). Java triggerExhaustedCardsOnStanceChange.
            break;
        }

        case CardId::CONSECRATE: {
            const int dmg = calculateCardDamage(c, -1, up ? 8 : 5);
            addToBot( Actions::AttackAllEnemy(dmg) );
            break;
        }

        case CardId::CRUSH_JOINTS: {
            const int dmg = calculateCardDamage(c, t, up ? 10 : 8);
            const int vulnAmt = up ? 2 : 1;
            const bool prevWasSkill = (previousCardType == CardType::SKILL);
            addToBot( Actions::AttackEnemy(t, dmg) );
            if (prevWasSkill) {
                addToBot( Actions::DebuffEnemy<MS::VULNERABLE>(t, vulnAmt, false) );
            }
            break;
        }
        ...
        case CardId::SASH_WHIP: {
            const int dmg = calculateCardDamage(c, t, up ? 10 : 8);
            const int weakAmt = up ? 2 : 1;
            const bool prevWasAttack = (previousCardType == CardType::ATTACK);
            addToBot( Actions::AttackEnemy(t, dmg) );
            if (prevWasAttack) {
                addToBot( Actions::DebuffEnemy<MS::WEAK>(t, weakAmt, false) );
            }
            break;
        }
        ...
        case CardId::WINDMILL_STRIKE: {
            // Java baseDamage 7, +3 on upgrade (= 10). When retained, the
            // onRetained hook adds +magic per retain (4 / 5 upgraded),
            // accumulated in misc.
            int bonusDmg = c.misc;
            const int baseDmg = up ? 10 : 7;
        ...
        case CardId::EMPTY_FIST: {
            const int dmg = calculateCardDamage(c, t, up ? 14 : 9);
            addToBot( Actions::AttackEnemy(t, dmg) );
        ...
        case CardId::JUST_LUCKY: {
            const int dmg = calculateCardDamage(c, t, up ? 4 : 3);
            addToBot( Actions::AttackEnemy(t, dmg) );
            addToBot( Actions::GainBlock(up ? 3 : 2) );
            addToBot( Actions::ScryAction(up ? 2 : 1) );
            break;
        }
...
void BattleContext::useSkillCard() {
    ...
    switch (c.getId()) {
        ...
        case CardId::FORCE_FIELD: {
            // Base cost is 4, reduced by 1 per power card played this combat
            // (cost reduction handled elsewhere). Block is 12 / 16.
            addToBot( Actions::GainBlock(calculateCardBlock(up ? 16 : 12)) );
            break;
        }
        ...
        case CardId::STEAM_BARRIER: {
            addToBot( Actions::GainBlock(calculateCardBlock(up ? 8 : 6)) );
            addToBot( Action([=] (BattleContext &b) {
        ...
        case CardId::PERSEVERANCE: {
            // Block bonus stored in misc (accumulates while retained)
            int bonusBlock = c.misc;
            addToBot( Actions::GainBlock(calculateCardBlock(up ? 7 : 5) + bonusBlock) );
            break;
        }
